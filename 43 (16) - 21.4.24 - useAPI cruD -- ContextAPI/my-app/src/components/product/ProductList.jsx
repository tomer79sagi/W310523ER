...
const ProductList = () => {
    ...
    const reducer = (state, action) => {
        switch (action.type) {
            ...
            case ACTION_TYPES.PRODUCT_DELETE:
                return { ...state, isDelete: true, selectedProduct: action.payload };

            case ACTION_TYPES.PRODUCT_DELETED:
                return { ...state, isDelete: false, products: state.products.filter(p => p.id !== action.payload.id)};
        }
    }
    ...
    useEffect(() => {
        callAPI(apiURL, METHOD.GET_ALL);
    }, []);

    useEffect(() => {
        if (state.isDelete) {
            callAPI(apiURL, METHOD.DELETE, {id: state.selectedProduct.id});
        }
    }, [state.isDelete]);
